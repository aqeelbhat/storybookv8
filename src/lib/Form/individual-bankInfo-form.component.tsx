...
import { Address, BankKey, EncryptedData, mapBankInfo, Option } from './../Types'
import styles from './bankInfo-form-styles.module.scss'
import { TextBox, ToggleSwitch, TypeAhead } from '../Inputs';
import { areObjectsSame, getEmptyAddress, getFormFieldConfig, isAddressInvalid, isDisabled, isEmpty, isRequired, isStateNeeded, mapBankAddress, mapBankCountryToOption, mapBankKeysToOption, mapBankKeyToOption, mapKeyLookupEntryToOption } from './util';
...
import { BankAccountType } from '../Types/common';
...
export function IndividualBankInfoForm (props: BankInfoFormProps) {
  ...
  const [internationalOptionalCode, setInternationalOptionalCode] = useState<string>('')
  ...
  useEffect(() => {
    if (props.formData) {
      ...
      // if (props.formData.bankInformation.bankAddress) {
      //   if (bankCountry) {
      ...
      setAccountType(props.formData.bankInformation.bankAccountType || BankAccountType.checking)
  ...
  useEffect(() => {
    if (props.countryOptions && props.formData) {
      ...
      // else if (props.formData.bankInformation.accountHolderAddress) {
  ...
  function getFormData (): BankInfoFormData {
    return {
      ...
      intermediaryBankInformation: (props.isCrossBorder && intermediaryBankRequired)
        ...
        : null,
      bankAccountTypeRequired: props.formData?.bankAccountTypeRequired,
  ...
  function getFormDataWithUpdatedValue (fieldName: string, newValue: string | Option | Address | EncryptedData | boolean): BankInfoFormData {
    ...
    switch (fieldName) {
      ...
      case 'accountHolderAddress':
        ...
        break
      case 'accountType':
          formData.bankInformation.bankAccountType = newValue as BankAccountType
          break
      case 'accountNumber':
      ...
      case 'internationalCode':
        ...
        formData.bankInformation.key=""
        break
      case 'internationalOptionalCode':
        formData.bankInformation.internationalCode = newValue as string
        formData.bankInformation.key=""
        break
  ...
  function isFormInvalid (): string {
    ...
    if (!isInvalid) {
      ...
      } else if (props.isCrossBorder && !internationalCode && !internationalCodeEncrypted) {
        isInvalid = true
        invalidFieldId = 'int-bank-code-field'
      } else if (props.formData.bankAccountTypeRequired && !accountType) {
        isInvalid = true
  ...
  function handleFormSubmit () {
    ...
    } else if (selectedBankDetailIndex > -1 || getSelectedBankDetails()) {
      const bankInfo = {...getFormData().bankInformation, ...existingBankList[selectedBankDetailIndex], ...{currencyCode: currencyCode}}
  ...
  function onAccountTypeChange (e) {
    ...
    setAccountType(type)
    handleFieldValueChange('accountType', accountType, type)
  }
  ...
  useEffect(() => {
    ...
    currencyCode, bankName, accountHolder, accountHolderAddress, accountNumber, bankAddress, bankCountry, accountType,
  ...
  return (
    ...
    <div className={styles.supplierBankInfoForm}>
      ...
      {showBankSelectionMethodOption && <div className={classnames(styles.section, styles.existingBankListSection, showBankDetailForm ? styles.noBorder : '', showBankDetailForm ? styles.noMargin : '')}>
        <ExistingBankListing
          ...
          {props.isCrossBorder && <div className={styles.row}>
            <div className={classnames(styles.item, props.isInPortal ? styles.col4 : styles.col3)} id="bank-address-field">
              <GoogleMultilinePlaceSearch
                id="bank-address"
                label={t("Bank location for payments")}
                value={bankAddress}
                countryOptions={props.countryOptions}
                required={true}
                forceValidate={forceValidate}
                validator={(value) => validateAddressField(t("Bank address"), value)}
        ...
        <div className={styles.section}>
          {bankAddress?.alpha2CountryCode && <>
            ...
            {!props.isCrossBorder && props.domesticKeyOptions && props.domesticKeyOptions.length > 0 && <div className={styles.row}>
              <div className={classnames(styles.item, props.isInPortal ? styles.col4 : styles.col3)} id="bank-key-field">
                {props.domesticKeyOptions && props.domesticKeyOptions.length > 0 && <div className={styles.customItem}>
                  <label className={styles.label}>{t("Select bank key applicable to you")} <Tooltip title={props.bankKeys?.find(enumVal => enumVal.code === key)?.description} ><Info size={16} cursor="pointer" color="var(--warm-neutral-shade-300)"/></Tooltip></label>
                  <DropdownControl
            ...
            {!props.isCrossBorder && <div className={styles.row}>
              ...
              {!bankCodeEncrypted && <div className={styles.customItem}>
                <TextBox
                    value={bankCode}
                    disabled={false}  
                    required={true}
                    forceValidate={props.bankCodeError || forceValidate}
                    validator={(value) => (props.bankCodeError && !isEmpty(value)) 
                      ? getI18Text("is invalid",{ label: props.bankKeys?.find(enumVal => enumVal.code === key)?.name || key || t("Bank code")}) : isEmpty(value) ? getI18Text("is required field",{label:t("Bank code")}) : ''}
            ...
            {props.isCrossBorder && (internationalKey === "iban" || (previousBankCode=== "iban" && bankAddress?.alpha2CountryCode === previousBankAddress?.alpha2CountryCode)) &&
              <div className={styles.row}>
                <div className={classnames(styles.item, props.isInPortal ? styles.col4 : styles.col3)} id="bank-key-options-field">
                  <label className={styles.label}>{t("Select bank key applicable to you")}
            ...
            {props.isCrossBorder &&
              <div className={styles.row}>
                ...
                {(internationalKey !== "iban" && previousBankCode !== "iban") && <label className={styles.label}>
                    {props.bankKeys?.find(enumVal => enumVal.code === internationalKey)?.name || internationalKey || t("International bank code")}
                  ...
                  {!internationalCodeEncrypted &&
                    <TextBox
                      ...
                      validator={(value) => (props.internationalCodeError) ? 
                        getI18Text("is invalid",{label:(props.bankKeys?.find(enumVal => enumVal.code === selectedInternationalkeyOption?.path)?.name || internationalKey || t("International bank code"))}) : isEmpty(value) ? getI18Text("is required field",{label:t("International bank code")}) : ''}
            ...
            {props.formData.bankAccountTypeRequired &&
              <div className={styles.row}>
                <div className={classnames(styles.item, props.isInPortal ? styles.col4 : styles.col3)} id="account-type-field">
                  <label className={styles.label}>{t("Account Type")}</label>
                  <RadioGroup aria-label="accountType" name="accountType" value={accountType || ''} onChange={onAccountTypeChange} className={styles.accountType} defaultValue={accountType}>
                    <FormControlLabel
                      value={BankAccountType.checking}
                      control={<Radio
                        id="checking"
                        checkedIcon={<img src={radioFilledGreenIcon} alt="" />}
                        icon={<Circle color="#d9d9d9" size={15}></Circle>} />
                      }
                      label={t("Checking")}
                    />
                    <FormControlLabel
            ...
            <div className={styles.row}>
              <div className={classnames(styles.item, styles.col3)} id="account-holder-address-field">
                <GoogleMultilinePlaceSearch
                  ...
                  required={true}
                  forceValidate={forceValidate}
                  validator={(value) => validateAddressField(t("Beneficiary Address"), value)}
            ...
            <div className={styles.row}>
              <div className={classnames(styles.item, props.isInPortal ? styles.col4 : styles.col3)}>
                <TextArea
                  label={t("Instruction")}
                  value={instruction}
                  onChange={value => { setInstruction(value); handleFieldValueChange(t("Instruction"), instruction, value) }}
                />
              </div>
            </div>
          ...
          { intermediaryBankRequired &&
            <div className={styles.row}>
              <div className={classnames(styles.item, props.isInPortal ? styles.col4 : styles.col3)} id="intermediary-bank-code-field">
                <TextBox
                  label={props.bankKeys?.find(enumVal => enumVal.code === intermediaryKey)?.name || intermediaryKey || t("Bank code")}
                  value={intermediaryBankCode}
                  disabled={false}
                  required={true}
                  forceValidate={props.intermediaryCodeError || forceValidate}
                  validator={(value) => props.intermediaryCodeError ? getI18Text("is invalid",{label:t("Intermediary bank code")}) : isEmpty(value) ? getI18Text("is required field",{label:t("Intermediary bank code")}) : ''}
