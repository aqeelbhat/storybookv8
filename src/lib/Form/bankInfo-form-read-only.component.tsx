...
import { AddressesValue, AddressValue, mapCustomFieldValue } from '../CustomFormDefinition/View/ReadOnlyValues'
import { NAMESPACES_ENUM, useTranslationHook } from "../i18n";
import { CustomFieldType } from '../CustomFormDefinition/types/CustomFormModel'

export function BankInfoFormReadOnly (props: {data: BankInfoFormData, bankKeys?: EnumsDataObject[], isSingleColumnLayout?: boolean, trackedAttributes?: TrackedAttributes}) {
    ...
    <div className={`oroFormReadOnly ${props.isSingleColumnLayout ? 'singleColumn' : ''}`}>
      ...
      <div className="formFields">
        <div className="keyValuePair">
          <div className="label">{t("Accounts receivable email")}</div>
           <div className="value">{mapCustomFieldValue({value:props.data.businessEmail, fieldName: 'businessEmail', trackedAttributes: props.trackedAttributes, fieldType: CustomFieldType.string}) || '-'}</div>
        </div>
        <div className="keyValuePair">
          <div className="label">{t("Payment/Remittance currency")}</div>
          <div className="value">
            {mapCustomFieldValue({value:props.data.bankInformation?.currencyCode?.displayName, fieldName: 'bankInformation', fieldValue: 'currencyCodeStr', trackedAttributes: props.trackedAttributes, fieldType: CustomFieldType.string}) || '-'}
          </div>
        </div>
      </div>
      ...
      <div className="formFields">
        ...
        {
          props.data?.bankInformation?.bankAddress && props.data?.bankInformation?.bankAddress?.alpha2CountryCode &&
          <div className="keyValuePair">
      ...
      {props.data.intermediaryBankRequired &&
        <div className="formFields">
          <div className="keyValuePair">
            ...
            {/* <div className="value">{props.data.intermediaryBankInformation?.bankName || '-'}</div> */}
          </div>
        ...
        {/* <div className="summary">{props.data.instruction || '-'}</div> */}
