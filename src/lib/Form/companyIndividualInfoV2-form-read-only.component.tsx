...
import { CompanyIndividualInfoV2FormData, EnumsDataObject, TaxType, TrackedAttributes } from './types'
...
export interface CompanyIndividualInfoV2FormReadOnlyProps {
...
export function CompanyIndividualInfoV2FormReadOnly (props: CompanyIndividualInfoV2FormReadOnlyProps) {
  ...
  function getFullName (): string {
    return `${props.data?.firstName || ''} ${props.data?.middleName || ''} ${props.data?.lastName || ''}`
  }

  function handleTaxValidation (data: EncryptedData, countryCode: string, taxCode: string, tax: string) {
    if (props.onValidateTax && getFullName() && data && countryCode) {
      props.onValidateTax(getFullName(), data, countryCode, taxCode, tax)
    }
  }
  ...
  return (
    <div className={`oroFormReadOnly ${props.isSingleColumnLayout ? 'singleColumn' : ''}`}>
      ...
      <div className="sectionTitle">{t("Individual Details")}</div>

      <div className="formFields">
        <div className="keyValuePair">
          <div className="label">{t("Full name")}</div>
          <div className="value">{mapCustomFieldValue({value:props.data.firstName, fieldName: 'firstName', trackedAttributes: props.trackedAttributes, fieldType: CustomFieldType.string}) || '-'} {mapCustomFieldValue({value:props.data.middleName, fieldName: 'middleName', trackedAttributes: props.trackedAttributes, fieldType: CustomFieldType.string}) || '-'} {mapCustomFieldValue({value:props.data.lastName, fieldName: 'lastName', trackedAttributes: props.trackedAttributes, fieldType: CustomFieldType.string}) || '-'}</div>
        </div> 
        ...
        <div className="keyValuePair">
          <div className="label">{t("Doing business as")}</div>
          <div className="value">{props.data?.companyName ? t("Company DBA entity") : t("Individual")}</div>
        </div>
      ...
      <div className="formFields">
        <div className="keyValuePair">
          ...
          <div className="value">{props.data?.address ? convertAddressToString(props.data?.address) : '-'}</div>
        </div>
      </div>
        ...
        <div className="formFields">
          <div className="keyValuePair">
            ...
            <div className="value">
              ...
              {
                props.isValidationSupported && props.data?.taxCodeValidationTimeout && props.data?.taxCodeError && (props.data.encryptedTaxCode?.maskedValue || props.data.encryptedTaxCode?.unencryptedValue) &&
                <div className="reverify" onClick={() => handleTaxValidation(props.data.encryptedTaxCode, props.data?.address?.alpha2CountryCode?.toLowerCase(), getTaxKeyNameForKey(props.data?.taxKey) || 'Tax code', TaxType.direct)}>{t("Reverify")}</div>
              }
            </div>
          </div>
        </div>}
      ...
      { props.data?.usTaxDeclarationFormKey && props.data?.usTaxDeclarationForm &&
        <div className="formFields">
          <div className="keyValuePair">
            <div className="label">{getTaxFormNameForKey(props.data?.usTaxDeclarationFormKey) || 'Tax Form'}</div>
            <div className="value">
              <AttachmentReadOnly
      ...
      { props.data?.taxKey && props.data?.taxForm &&
        ...
        <div className="formFields">
          <div className="keyValuePair">
            <div className="label">{t("Any special status?")}</div>
      ...
      { props.data?.exemptionTaxKey &&
        <div className="formFields">
            ...
            <div className="value">
                ...
                {
                  props.isValidationSupported && props.data?.exemptionTaxCodeValidationTimeout && props.data?.exemptionTaxCodeError && (props.data.encryptedExemptionTaxCode?.maskedValue || props.data.encryptedExemptionTaxCode?.unencryptedValue) &&
                  <div className="reverify" onClick={() => handleTaxValidation(props.data.encryptedIndirectTaxCode, props.data?.address?.alpha2CountryCode?.toLowerCase(), getTaxKeyNameForKey(props.data?.exemptionTaxKey) || 'Tax Code', TaxType.exemption)}>Reverify</div>
                }
            </div>
            </div>
        </div>}
      ...
      <div className="formFields">
        <div className="keyValuePair">
          ...
          <div className="value">{props.data.instruction || '-'}</div>
        </div>
      </div>
    </div>
  )
